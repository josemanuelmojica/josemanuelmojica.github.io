...
The repository now includes a browser studio that turns real poster road geometry into reusable, text-free animation assets. The full map stays visible while a controlled subset of streets marches into a ring or spiral and returns exactly home.
...
Open `http://127.0.0.1:5000`. The included sunset street study contains 80,074 source paths, 3,000 spatially distributed motion candidates, and defaults to animating 120 pieces. Generate a new place from the studio, or build an asset directly:

```bash
node packages/map-pieces/scripts/build-motion-asset.js input.svg public/maps study-name 3000
```

Package contracts and integration decisions are documented in [`packages/map-pieces`](packages/map-pieces), [`packages/map-motion`](packages/map-motion), [`experiments/ant-motion/INTEGRATION.md`](experiments/ant-motion/INTEGRATION.md), and [`docs/ADR-002-product-motion-assets.md`](docs/ADR-002-product-motion-assets.md).

### Street Choreographies / Volume 01

The curated design set lives in [`design/city-collection.json`](design/city-collection.json). It contains eight label-free studies: San Francisco, San Diego, Portland, New York City, Austin, Chicago, Minneapolis, and Charlotte. Build or resume the assets in parallel by passing non-overlapping study IDs:

```bash
npm run build:city-collection -- study-01 study-02
npm run build:city-collection -- study-03 study-04
npm run build:city-collection -- study-05 study-06
npm run build:city-collection -- study-07 study-08
npm run build:city-collection -- --merge-only
```

The public review board is `street-choreographies-v1.html` after `npm run build`.
