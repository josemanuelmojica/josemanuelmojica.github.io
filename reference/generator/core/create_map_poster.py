...
# Public Overpass instances occasionally become unavailable. Allow batch asset
# builds and deployments to select a healthy compatible endpoint without
# changing generator code.
if overpass_url := os.environ.get("OVERPASS_URL"):
    ox.settings.overpass_url = overpass_url.rstrip("/")
if os.environ.get("OVERPASS_RATE_LIMIT") == "0":
...
def create_poster(
    ...
    skipped_features = {value.strip().lower() for value in os.environ.get("SKIP_OSM_FEATURES", "").split(",") if value.strip()}
    ...
    with tqdm(
        ...
        water = None if "water" in skipped_features else fetch_features(
            point,
            compensated_dist,
        ...
        parks = None if "parks" in skipped_features else fetch_features(
            point,
            compensated_dist,
    ...
    if os.environ.get("HIDE_LABELS") == "1":
