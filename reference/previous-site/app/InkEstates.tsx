...
const studies: Study[] = [
  {
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-01.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-01.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-01.active.json",
  },
  {
    id: "study-02",
    city: "San Diego",
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-02.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-02.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-02.active.json",
  },
  {
    id: "study-03",
    city: "Portland",
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-03.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-03.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-03.active.json",
  },
  {
    id: "study-04",
    city: "New York City",
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-04.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-04.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-04.active.json",
  },
  {
    id: "study-05",
    city: "Austin",
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-05.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-05.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-05.active.json",
  },
  {
    id: "study-06",
    city: "Chicago",
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-06.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-06.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-06.active.json",
  },
  {
    id: "study-07",
    city: "Minneapolis",
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-07.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-07.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-07.active.json",
  },
  {
    id: "study-08",
    city: "Charlotte",
    ...
    baseUrl: "/maps/japanese-ink-scroll/base/study-08.webp",
    overlayUrl: "/maps/japanese-ink-scroll/study-08.active-overlay.svg",
    metadataUrl: "/maps/japanese-ink-scroll/study-08.active.json",
  },
];
...
const listings: Listing[] = [
  {
    ...
    baths: 7,
    area: "8,410 sq ft",
    image: "/properties/residence-03.jpg",
  ...
  },
  {
    ...
    baths: 8,
    area: "7,920 sq ft",
    image: "/properties/residence-02.jpg",
  ...
  },
  {
    ...
    baths: 5,
    area: "5,860 sq ft",
    image: "/properties/residence-01.jpg",
    ...
    () => ({
      update(amount: number) {
        ...
        for (const piece of piecesRef.current) {
          ...
          const dx = sealX - piece.centerX;
          const dy = sealY - piece.centerY;
          piece.element.style.transformOrigin = `${piece.centerX}px ${piece.centerY}px`;
          piece.element.style.transform = `translate(${dx * translated}px, ${dy * translated}px) scale(${1 - 0.18 * translated})`;
      ...
      .then(([source, metadata]) => {
        ...
        piecesRef.current = metadata.pieces.slice(0, limit).flatMap((piece) => {
          ...
          return [
            {
              element,
              centerX: piece.centroid[0],
              centerY: piece.centroid[1],
    ...
    <div className="map-layer">
      {!baseFailed ? (
        <img
          ...
          src={study.baseUrl}
          alt={`${study.city}, ${study.state} street network in Japanese ink`}
          onError={() => setBaseFailed(true)}
        />
      ) : (
        <div className="map-layer__fallback" role="status">
...
function MarketStory({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  useEffect(() => {
    ...
    let frame = 0;
    let storyTop = 0;
    let chapterSpan = window.innerHeight * (mobile ? 1.25 : 1.45);

    const measure = () => {
      const story = storyRef.current;
      if (!story) return;
      storyTop = story.getBoundingClientRect().top + window.scrollY;
      chapterSpan = window.innerHeight * (mobile ? 1.25 : 1.45);
    };

    const paint = () => {
      frame = 0;
      const raw = clamp(
        (window.scrollY - storyTop) / chapterSpan,
        0,
        studies.length,
      );
      const nextIndex = Math.min(Math.floor(raw), studies.length - 1);
      const local = clamp(raw - nextIndex);
  ...
  }, [activeIndex, mobile, reducedMotion]);

  return (
    <section className="market-story" id="markets" ref={storyRef}>
      <div className="market-stage paper-stage" aria-hidden="true">
        <div className="market-stage__counter">
          <span>{String(activeIndex + 1).padStart(2, "0")}</span>
          <i />
          <span>{String(studies.length).padStart(2, "0")}</span>
        </div>
        <div className="map-shell map-shell--outgoing" ref={outgoingShellRef}>
          <MapLayer
            ref={outgoingRef}
            study={current}
            direction="outgoing"
        ...
        {activeIndex < studies.length - 1 && (
          <div className="map-shell map-shell--incoming" ref={incomingShellRef}>
            <MapLayer
              ref={incomingRef}
              study={next}
              direction="incoming"
        ...
        <div className="ink-seal"><span>χ</span></div>
        <div className="market-stage__label">
      ...
      <div className="market-story__timeline">
        {studies.map((study, index) => (
          <section className="market-chapter" key={study.id}>
            <div className="market-chapter__copy">
              ...
              <a href="#properties" aria-label={`Explore representative homes in ${study.city}`}>
...
function BrandName() {
  return (
    <span className="wordmark__art" aria-hidden="true">
      <img src="/brand/ark-and-text-source.png" alt="" />
    </span>
  );
}
  ...
  return (
    <main>
      ...
      <section className="hero paper-stage" id="top">
        <img
          className="hero__map"
          src="/maps/japanese-ink-scroll/base/study-01.webp"
      ...
      <section className="properties" id="properties">
        ...
        <div className="filter-line" aria-label="Property filters">
          <img className="freehand-icon filter-line__icon" src="/icons/freehand-filter.png" alt="" />
          <label>
            <span>Showing</span>
            <select value={marketFilter} onChange={(event) => setMarketFilter(event.target.value)}>
              <option>All markets</option>
      ...
      <section className="approach" id="approach">
        ...
        <div className="approach__steps">
          <article>
            <img className="freehand-icon" src="/icons/freehand-key.png" alt="" />
            <span>一 / Listen</span>
            <h3>Start with Tuesday.</h3>
            <p>Tell us how you spend a normal week. We’ll work backward to the house and the neighborhood.</p>
          </article>
          <article>
            <img className="freehand-icon" src="/icons/freehand-filter.png" alt="" />
            <span>二 / Edit</span>
          ...
          <article>
            <img className="freehand-icon" src="/icons/freehand-calendar.png" alt="" />
            <span>三 / Steward</span>
      ...
      <section className="contact paper-stage" id="contact">
        <img
          src="/maps/japanese-ink-scroll/base/study-08.webp"
          alt="Charlotte street network rendered as a Japanese ink drawing"
        />
        <div className="contact__copy">
          <img className="freehand-icon contact__icon" src="/icons/freehand-calendar.png" alt="" />
        ...
        {inquirySent ? (
          <div className="inquiry-success" role="status">
            <span className="wordmark__seal"><img src="/icons/freehand-calendar.png" alt="" /></span>
