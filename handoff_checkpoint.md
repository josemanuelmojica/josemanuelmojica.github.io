...
- **Cloudflare Preview:** Immutable version 19 is `https://64137e6e-ark-and-text.j-m-mojica-g.workers.dev`. It corresponds to commit `45e9abf`, contains the tested export, D1 and encrypted secret bindings, fails closed without the Turnstile secret, and binds proof to the exact action, frontend hostname, and per-form UUID. Homepage/header, health, build identity, bindings, and D1 table checks pass. The synthetic interactive submission/D1-row check is still open. Production remains 100% on the previous version.
