...
- **Cloudflare Preview:** Immutable version 19 is `https://64137e6e-ark-and-text.j-m-mojica-g.workers.dev`. It corresponds to commit `45e9abf`, contains the tested export, D1 and encrypted secret bindings, fails closed without the Turnstile secret, and binds proof to the exact action, frontend hostname, and per-form UUID. Homepage/header, health, build identity, bindings, D1 tables, Boise → Idaho rendering, real Turnstile proof, public success state, and one-row D1 delivery checks pass. The synthetic row is marked `test`, and the schema has no raw-IP column. Production remains 100% on the previous version and requires explicit owner approval to promote.
...
- [x] Test a real Turnstile-protected synthetic submission on Cloudflare preview 19 and inspect the resulting D1 row without exposing contact fields.
- [x] Create `CLAUDE_REVIEW_PROMPT.md` with a constrained review/add/polish brief and current preview/repository facts.
