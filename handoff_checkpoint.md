...
- **Cloudflare Preview:** Immutable version 17 is `https://7b13b5ea-ark-and-text.j-m-mojica-g.workers.dev`. It contains the final tested source/export, D1 binding, encrypted Turnstile/rate-limit secrets, action-bound Turnstile verification, and protected `/api/lead`. Homepage/header, health, and CORS checks pass. Production remains 100% on the previous version; the interactive submission/D1-row check is still open.
